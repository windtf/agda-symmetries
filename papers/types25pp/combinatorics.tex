...
%
\begin{definition}[\alink{definition}{length}]
      \label{def:length}
...
%
\begin{definition}[\alink{definition}{$\in$}]
...
(the colloquial~$\term{here}/\term{there}$ constructors for de Bruijn indices).
...
\begin{definition}[\alink{definition}{$\term{Any}$ and $\term{All}$}]
      \label{def:any-all}
      \begin{align*}
