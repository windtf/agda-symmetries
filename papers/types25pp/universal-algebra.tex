...
\begin{definition}[\alink{definition}{Signature}]
    ...
    \label{def:signature}
    A signature, denoted $\sig$, is a (dependent) pair of:
    \begin{itemize}
        \item a set of operations, $\op\colon \Set$,
        \item an arity function for each operation, $\ar\colon \op \to \Set$.
    \end{itemize}
\end{definition}
...
\vc{inconsistent use of $\Set$ and $\UU$}
...
\begin{definition}[\alink{definition}{Structure}]
    ...
    A $\sig$-structure $\str{X}$ is an $\Sig$-algebra, that is, a pair of:
    \begin{itemize}
...
they are missing unit and associativity laws.
\begin{toappendix}
...
\begin{definition}[\alink{definition}{Equational Signature}]
    ...
    An equational signature, denoted $\eqsig$, is a pair of:
    \begin{itemize}
...
which, by construction, gives a homomorphism from $\str{F}(V)$ to $\str{X}$.
%
...
\end{definition}
%
The variety of algebras satisfying a given system of equations is a full subcategory of $\sigAlg$.
%
