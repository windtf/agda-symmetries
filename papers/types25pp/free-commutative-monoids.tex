...
\begin{definition}[\alink{definition}{Permutation relation}{permutation-relation}]
    ...
    \begin{itemize}
        ...
        \item a congruence wrt $\mult$: $\forall a, b, c, d, \, a \approx b \to c \approx d \to a \mult c \approx b \mult d$,
...
\begin{proof}
    ...
    And, $\exthat{f}(q(x) \qlift{\mult} q(y)) = \exthat{f}{(q(x \mult y))} = \ext{f}(x \mult y) = \ext{f}(x) \mult \ext{f}(y)$,
