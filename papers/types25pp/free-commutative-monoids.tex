...
\begin{definition}[\alink{definition}{Permutation relation}]
    ...
    \leavevmode
    A binary relation on a free monoid $(F(A), \mult)$ is a permutation relation iff it:
    \begin{itemize}
...
Let $q : F(A) \twoheadrightarrow \quot{F(A)}{\approx}$ be the quotient map.
%
...
\end{proof}
\noindent
As a consequence, there is at most one permutation relation on $F(A)$.
...
\begin{definition}[\alink{definition}{PList}]
    ...
    \begin{align*}
        \term{perm-refl} & : \forall \{xs\} \to \Perm\,xs\,xs \\
        \term{perm-swap} & : \forall \{x\,y\,xs\,ys\,zs\}
        \to \Perm\,(xs \concat x \cons y \cons ys)\,zs
