...
%
The constructions in~\cref{sec:cmon:plist} and~\cref{cmon:bag} are specific instances of this recipe.
...
\begin{definition}[\alink{definition}{Permutation relation}]
    \label{def:permutation-relation}
    \leavevmode
    A binary relation on a free monoid $(F(A), e, \mult)$ is a permutation relation iff it:
    \begin{itemize}
...
the identity element is $q(e)$,
and the multiplication operation $\mult$ is lifted to the quotient by congruence.
...
%
\begin{toappendix}
    For clarity, we use $\exthat{(\blank)}$ to denote the extension operation of $F(A)$,
    and $\ext{(\blank)}$ for the extension operation of $\quot{F(A)}{\approx}$.
\end{toappendix}
%
\begin{definition}[\alink{definition}{}]
    Given a commutative monoid $\str{X}$ and a map $f : A \to X$,
    we define the extension operation as follows:
    we first obtain $\exthat{f} : \str{F}(A) \to \str{X}$ by the universal property of $F$,
    then lift it to $\ext{f} : \quot{\str{F}(A)}{\approx} \; \to \str{X}$,
    by the properties of $\approx$ in~\cref{def:permutation-relation}.
\end{definition}

\begin{propositionrep}[\alink{proposition}{Universal property for $\quot{\str{F}(A)}{\approx}$}]
    \label{prop:qfreemon}
    \leavevmode
    $(\quot{\str{F}(A)}{\approx}, \eta_A : {A \xto{\eta_A} \str{F}(A) \xto{q} \quot{\str{F}(A)}{\approx}})$
    forms the free commutative monoid on $A$.
\end{propositionrep}
\begin{proof}
...
we consider a simple one which swaps any two adjacent elements somewhere in the middle of the list.
...
%
\begin{definition}[\alink{definition}{PList}]
...
\end{proof}
%
This particular representation makes it easy to lift functions and properties defined on $\List$
to $\PList$ since $\PList$ is a quotient of $\List$.
%
The inductive nature of $\PList$ makes it easy to define algorithms and proofs that are inductive in nature,
e.g. defining insertion sort on $\PList$ is simple since insertion sort inductively sorts a list,
which we can do easily by pattern matching on $\PList$.
%
...
\begin{definition}[\alink{definition}{Bag}]
    \label{def:bag}
    \leavevmode
    \begin{code}
...
We have already given a proof of $\Array$ being the free monoid in~\cref{sec:arrays}.
%
By~\cref{prop:qfreemon} it suffices to show $\approx$ satisfies the axioms of permutation relations to show that $\Bag$
...
\begin{theorem}[\alink{theorem}{Permutation invariance}]
    \label{bag:perm-sat}
