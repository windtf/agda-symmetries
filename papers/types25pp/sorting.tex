...
\begin{proof}
    ...
    \end{itemize}
\end{proof}
Alternatively, we can define an irreflexive linear order $<$ on $A$.
\begin{definition}[\alink{definition}{Linear order}]
    \label{def:linear-order}
    A linear order on a set $A$ is a relation $< : A \to A \to \hProp$ that satisfies:
    \begin{itemize}
        \item irreflexivity: $\neg(x < x)$,
    ...
    \end{itemize}
    Note that a strict order is simply a linear order without the connectedness axiom.
    A \emph{decidable} linear order requires the $<$ relation to be decidable:
    \begin{itemize}
...
\begin{proposition}[\alink{proposition}{}]
    ...
    In a decidable linear order, it holds that ${\forall x, y}, \ps{x < y} + \ps{y < x}$.
    Further, this makes $A$ discrete, that is ${\forall x, y}, \ps{x \id y} + \ps{x \neq y}$.
\end{proposition}
\begin{proof}
...
\begin{proposition}[\alink{proposition}{}]
    ...
    The set of decidable linear orders on $A$ and the set of decidable total orders on $A$ are equivalent.
\end{proposition}
\begin{proofsketch}
    Given a decidable total order $\leq$ on $A$, by~\cref{prop:decidable-total-order},
    we can map to a decidable linear order $<$ on $A$ by
    $x < y \defeq (x \leq y) \times (x \neq y)$. Vice versa, by~\cref{prop:decidable-linear-order}
    given a decidable linear order $<$ on $A$,
...
\begin{corollary}[\alink{corollary}{}]
    ...
    Let $\term{DecLinOrd}(A)$ be the set of decidable linear orders on $A$,
