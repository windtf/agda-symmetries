...
\begin{proposition}[\alink{proposition}{}]
    \label{sort:declord-to-deceq}
    Assume $A$ has a decidable strict total order $<$,
    then $A$ has decidable equality.
\end{proposition}
\begin{proof}
...
\end{corollary}
\begin{proof}
    Given a decidable total order $\leq$ on $A$, by~\cref{sort:decord-to-deceq},
    we can construct a decidable strict linear order $<$ on $A$ by
    $x < y \defeq x \leq y \land x \neq y$. Vice versa, by~\cref{sort:declord-to-deceq}
    given a decidable strict linear order $<$ on $A$,
    we can construct a decidable total order $\leq$ on $A$ by
    $x \leq y \defeq x < y \lor x = y$. Thus, we have an equivalence
