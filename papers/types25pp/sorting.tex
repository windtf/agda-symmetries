...
\begin{definition}[\alink{definition}{$\term{min}$}{min-free-commutative-monoid}]
    \label{def:min-free-commutative-monoid}
    ...
    \[
        ...
        \end{array}
    \]
    This gives a homomorphism \({\term{min} \defeq \ext{\inr}} : {\MM(A) \to 1 + A}\),
...
\begin{corollary}[\alink{corollary}{}{lattice}]
    ...
    Let $\term{TMSLattice}(A)$ be the set of total meet semilattices on $A$,
    There is a map $l2s \colon \term{TMSLattice}(A) \to \term{Sort}(A)$,
    which is an equivalence.
\end{corollary}
\begin{proof}
    ...
    $\term{TMSLattice}(A) \simeq \term{DecTotOrd}(A)$, and the result follows from~\cref{sort:main}
    by transitivity of equivalences.
\end{proof}
%
