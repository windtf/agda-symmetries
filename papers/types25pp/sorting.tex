...
Classically, assuming the Axiom of Choice would allow us to construct a section (right-inverse, in $\Set$) to the surjection $q$,
that is,
...
by $\term{swap}$.
%
Since $\forall \xs.\,q(s(\xs)) \id \xs$, $s$ must preserve all the elements of $\xs$.
...
\begin{proofsketch}
    ...
    To see how this proposition holds: $q(s(\xs))$ first orders an unordered list $\xs$ by $s$,
    then discards the order again by $q$ --
...
\end{proof}
%
A crucial observation is that $s$ correctly orders 2-element bags,
...
\end{proposition}
\begin{proof}
    We give a counterexample of an $s$ that violates transitivity.
    ...
    Then we use $\term{sort}$ to construct $s$.
    \begin{align*}
        s(\xs) & = \begin{cases}
    ...
    \end{align*}
    %
    Now we have $s([2,3,1]) = [1,2,3]$ but $s([2,3,1,4]) = [4,3,2,1]$.
\end{proof}
%
We will enforce additional constraints on the \emph{image} of $s$, to turn it into a correct sort function.
\begin{definition}[\alink{definition}{$\blank\in\im{s}$}]
    \label{def:in-image}
    The fiber of $s$ at a point in the codomain~$\xs : \LL(A)$
    is given by $\fib_{s}(\xs) \defeq \dsum{ys : \MM(A)}{(s(ys) = \xs)}$.
    %
