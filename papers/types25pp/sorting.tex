...
\end{proposition}
\begin{proof}
    We give a counter-example of a $\ssection$ that satisfies $\isheadleast$ but is not a sort function.
...
\begin{lemma}[\alink{lemma}{}{is-sorted-unique}]
    ...
    $\quotient(\xs) = \quotient(\ys) \land \isSorted_{\leq}(\xs) \land \isSorted_{\leq}(\ys) \to \xs = \ys$.
\end{lemma}
%
Insertion sort by $\leq$ always produces lists that satisfy $\isSorted_{\leq}$.
Functions that also produce lists satisfying $\isSorted_{\leq}$ are equal to insertion sort
by function extensionality.
%
\begin{proposition}[\alink{proposition}{}{sort-uniq}]
    \label{sort:sort-uniq}
    Given a total order $\leq$,
    if a section $\ssection$ always a produces sorted list, i.e. $\forall \xs.\,\isSorted_{\leq}(\ssection(\xs))$,
