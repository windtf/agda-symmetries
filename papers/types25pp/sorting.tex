...
First, we recall the axioms of a total order $\leq$ on a set $A$. This is also known as a
linear order.
\begin{definition}[\alink{definition}{Total order}]
    ...
    \end{itemize}
\end{definition}
This strengthens the totality axiom,
...
\begin{proof}
    ...
    \end{itemize}
\end{proof}
We also recall the axioms of a strict total order $<$ on $A$. For clarity, we call this a
a \emph{strict linear order} to distinguish it from the non-strict total order defined above.
\begin{definition}[\alink{definition}{Strict linear order}]
    \label{def:linear-order}
    A strict linear order on a set $A$ is a relation $< : A \to A \to \hProp$ that satisfies:
    \begin{itemize}
        \item irreflexivity: $\neg(x < x)$,
    ...
    \end{itemize}
    A \emph{decidable} strict linear order requires the $<$ relation to be decidable:
    \begin{itemize}
...
\begin{proposition}[\alink{proposition}{}]
    \label{prop:decidable-linear-order}
    In a decidable strict linear order, it holds that ${\forall x, y}, \ps{x < y} + \ps{y < x}$.
    Further, this makes $A$ discrete, that is ${\forall x, y}, \ps{x \id y} + \ps{x \neq y}$.
\end{proposition}
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item
              if $x < y$ or $y < x$: by irreflexivity, $x \neq y$.
    \end{itemize}
\end{proof}

\begin{proposition}[\alink{proposition}{}]
    \label{prop:dec-linear-total-order}
    The set of decidable strict linear orders on $A$ and the set of decidable total orders on $A$ are equivalent.
\end{proposition}
\begin{proofsketch}
    Given a decidable total order $\leq$ on $A$, by~\cref{prop:decidable-total-order},
    we can map to a decidable strict linear order $<$ on $A$ by
    $x < y \defeq (x \leq y) \times (x \neq y)$. Vice versa, by~\cref{prop:decidable-linear-order}
    given a decidable strict linear order $<$ on $A$,
    we can map to a decidable total order $\leq$ on $A$ by
    $x \leq y \defeq (x < y) + (x = y)$. We can then show that these maps are inverses
    of each other.
\end{proofsketch}
...
\begin{definition}[\alink{definition}{Meet semi-lattice}]
    ...
    \end{itemize}
    A \emph{strongly-connected} meet semi-lattice further satisfies: \ww{Are the names right?}
    \begin{itemize}
...
%
The core question we want to study is the existence of $s$ in a constructive
...
Viewing the quotienting relation as a permutation relation (from~\cref{sec:cmon:qfreemon}), a section $s$ to $q$ has to pick out
...
%
Thus, if AC was assumed, we could easily recover an order on $A$ from the section $s$.
%
...
Thus, we will enforce this by imposing additional constraints on the \emph{image} of $s$.
\begin{definition}[\alink{definition}{$\blank\in\im{s}$}]
    \label{def:in-image}
...
Informally, it says that the head of an $s$-``sorted'' list
...
\begin{proposition}[\alink{proposition}{}]
    ...
    if a section $s$ always produces sorted list, i.e. $\forall \xs.\,\term{IsSorted}_{\leq}(s(\xs))$,
...
\begin{corollary}[\alink{corollary}{}]
    ...
    Let $\term{DecLinOrd}(A)$ be the set of decidable strict linear orders on $A$,
    There is a map $l2s \colon \term{DecLinOrd}(A) \to \term{Sort}(A) \times \term{Discrete}(A)$,
    which is an equivalence.
\end{corollary}
\begin{proof}
    By~\cref{prop:dec-linear-total-order}, we have an equivalence
