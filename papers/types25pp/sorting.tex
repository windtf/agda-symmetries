...
\begin{lemma}[\alink{lemma}{}]
    ...
    $\quotient(\xs) = \quotient(\ys) \land \term{IsSorted}_{\leq}(\xs) \land \term{IsSorted}_{\leq}(\ys) \to \xs = \ys$.
\end{lemma}
...
\begin{proposition}[\alink{proposition}{}]
    ...
    if a section $\ssection$ always produces sorted list, i.e. $\forall \xs.\,\term{IsSorted}_{\leq}(\ssection(\xs))$,
...
\begin{proposition}[\alink{proposition}{}]
    ...
    it holds that $\term{IsSorted}_{\leqs}(\ssection(\xs))$.
    %
