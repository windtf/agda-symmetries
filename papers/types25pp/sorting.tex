...
\begin{proposition}[\alink{proposition}{}{counterexample-equivalence}]
    ...
    Assume $A$ is a set with distinct elements, i.e. $\exists x, y: A.\,x \neq y$,
    then there is no equivalence between sections satisfying $\isheadleast$
    and total orders on $A$.
\end{proposition}
\begin{proof}
    We give a counterexample of an $\ssection$ that satisfies $\isheadleast$ but is not a sort function.
    Consider the insertion sort function $\term{sort} : \MM(\Nat) \to \LL(\Nat)$
    parameterized by $\leq$:
    \begin{align*}
        \term{reverse}([])          & = []                               \\
        \term{reverse}(x \cons \xs) & = x \cons \term{reverse}(\xs)      \\
        \ssection(\xs)              & = \term{reverse}(\term{sort}(\xs)) \\
