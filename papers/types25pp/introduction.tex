...
%     from other formalisations of the correctness of sorting such as
...
% We give a table of our formalisation in~\cref{sec:formalisation}.
...
% If we write a correct sort function $\ssection$, we will get a total order on $A$.
...
% The common formalisation of a correct sort algorithm is to say a sort algorithm produces
% an "orederd list", for example, one such formalisation would be the definition
...
% \end{lstlisting}
%
% This is a perfectly valid formalisation of an ordered list! Empty lists and singleton lists
...
\begin{myitemize}
  ...
  \item In~\cref{sec:monoids}, we give constructions of free monoids, and proofs of their universal property.
        ...
        These are standard constructions that we formalize conceptually by formal combinatorics.
  ...
  \item All the results of this paper are formalized in Cubical Agda, which is discussed in~\cref{sec:formalisation}.
...
formalisation, but to present the results in un-formalized form (in type-theoretic foundations), so the ideas are
accessible to a wider audience.
%
\ifarxiv{}{The extended version of this paper including additional formalisation details and proofs is available on arXiv.}i
%
The formalisation artifact is available at~\cite{choudhuryAgdasymmetries2025}.