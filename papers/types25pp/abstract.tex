...
As mathematical functions, they are simply endofunctions on lists,
that perform combinatorial operations on the representation of the input list.
...
Introducing symmetry by passing from free monoids (ordered lists) to free commutative monoids (unordered lists) eliminates ordering,
while sorting (the right inverse) recovers ordering.
From this, we give an axiomatization of sorting
which does not require a pre-existing total order on the underlying set,
and then show that there is an equivalence between
(decidable) total orders on the underlying set and correct sorting functions.
