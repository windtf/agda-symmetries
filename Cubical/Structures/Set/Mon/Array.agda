...
⊎InlB : (B : Type ℓ) (f : A -> C) (g : B -> C) -> (a : A) -> ⊎.rec f g (inl a) ≡ f a
⊎InlB B f g a = refl

⊎InrB : (A : Type ℓ) (f : A -> C) (g : B -> C) -> (b : B) -> ⊎.rec f g (inr b) ≡ g b
⊎InrB A f g b = refl

⊎E : {f : A -> C} {g : B -> C} -> (h : A ⊎ B -> C) (h1 : f ≡ h ∘ inl) (h2 : g ≡ h ∘ inr) -> ⊎.rec f g ≡ h
⊎E h h1 h2 i (inl a) = h1 i a
⊎E h h1 h2 i (inr b) = h2 i b
...
finSplit∘finCombine m n =
  ⊎.elim (\(k , k<m) ->
         ...
         ≡⟨ congS (finSplit m n) (⊎InlB (Fin n) finCombine-inl finCombine-inr (k , k<m)) ⟩
         ...
         ≡⟨ congS (finSplit m n) (⊎InrB (Fin m) finCombine-inl finCombine-inr (k , k<n)) ⟩
...
finCombine∘finSplit m n (k , k<m+n) =
  ⊎.rec (\k<m ->
        ...
        ≡⟨ ⊎InlB (Fin n) finCombine-inl finCombine-inr (k , k<m) ⟩
        ...
        ≡⟨ ⊎InrB (Fin m) finCombine-inl finCombine-inr (k ∸ m , ∸-<-lemma m n k k<m+n m≤k) ⟩
...
⊕-unitl (n , f) = Array≡ (zero+ n) \k k<n ->
    ⊎.rec finAbsurd f (finSplit 0 n (k , subst (k <_) (sym (zero+ n)) k<n))
  ≡⟨ congS (⊎.rec finAbsurd f) (finSplit-beta-inr k (subst (k <_) (sym (zero+ n)) k<n) zero-≤ k<n) ⟩
    ⊎.rec finAbsurd f (inr (k , k<n))
  ≡⟨ ⊎InrB (Fin 0) finAbsurd f (k , k<n) ⟩
    f (k , k<n)
  ∎

⊕-unitr : ∀ {ℓ} {A : Type ℓ} -> (xs : Array A) -> xs ⊕ e ≡ xs
⊕-unitr (n , f) = Array≡ (+-zero n) \k k<n ->
    ⊎.rec f finAbsurd (finSplit n 0 (k , subst (k <_) (sym (+-zero n)) k<n))
  ≡⟨ congS (⊎.rec f finAbsurd) (finSplit-beta-inl k k<n (subst (k <_) (sym (+-zero n)) k<n)) ⟩
    ⊎.rec f finAbsurd (inl (k , k<n))
  ≡⟨ ⊎InlB (Fin 0) f finAbsurd (k , k<n) ⟩
    f (k , k<n)
  ∎
...
assocr∘∸ : ∀ (k n m o : ℕ) -> k < n + (m + o) -> n + m ≤ k -> k ∸ (n + m) < o
assocr∘∸ k n m o p q = ∸-<-lemma (n + m) o k (subst (k <_) (+-assoc n m o) p) q

⊕AssocRLeftB : ∀ {ℓ} {A : Type ℓ} {m n o : ℕ} (as : Fin m -> A) (bs : Fin n -> A) (cs : Fin o -> A)
                      -> (k : ℕ) (k<m+n : k < m + n) (k<m+n+o : k < m + (n + o))
                      -> ⊎.rec as (⊎.rec bs cs ∘ finSplit n o) (finSplit m (n + o) (k , k<m+n+o)) ≡ ⊎.rec as bs (finSplit m n (k , k<m+n))
⊕AssocRLeftB {m = m} {n = n} {o = o} as bs cs k k<m+n k<m+n+o =
  ⊎.rec (\k<m ->
        ...
        ≡⟨ ⊎InlB (Fin (n + o)) as (⊎.rec bs cs ∘ finSplit n o) (k , k<m) ⟩
          as (k , k<m)
        ≡⟨ ⊎InlB (Fin n) as bs (k , k<m) ⟩
          ⊎.rec as bs (inl (k , k<m))
        ...
        ≡⟨ ⊎InrB (Fin m) as (⊎.rec bs cs ∘ finSplit n o) (k ∸ m , ∸-<-lemma m (n + o) k k<m+n+o m≤k) ⟩
        ...
        ≡⟨ ⊎InlB (Fin o) bs cs (k ∸ m , ∸-<-lemma m n k k<m+n m≤k) ⟩
          bs (k ∸ m , ∸-<-lemma m n k k<m+n m≤k)
        ≡⟨ ⊎InrB (Fin m) as bs (k ∸ m , ∸-<-lemma m n k k<m+n m≤k) ⟩
...
⊕AssocR (n , as) (m , bs) (o , cs) = Array≡ (sym (+-assoc n m o)) \k k<n+m+o ->
  ⊎.rec (\k<n+m ->
        ...
        ≡⟨ ⊎InlB (Fin o) (⊎.rec as bs ∘ finSplit n m) cs (k , k<n+m) ⟩
          ⊎.rec as bs (finSplit n m (k , k<n+m))
        ≡⟨ sym (⊕AssocRLeftB as bs cs k k<n+m k<n+m+o) ⟩
          ⊎.rec as (⊎.rec bs cs ∘ finSplit m o) (finSplit n (m + o) (k , k<n+m+o))
        ∎)
        (\n+m≤k ->
          ⊎.rec (⊎.rec as bs ∘ finSplit n m) cs (finSplit (n + m) o (k , subst (k <_) (+-assoc n m o) k<n+m+o))
        ≡⟨ congS (⊎.rec (⊎.rec as bs ∘ finSplit n m) cs) (finSplit-beta-inr k (subst (k <_) (+-assoc n m o) k<n+m+o) n+m≤k (assocr∘∸ k n m o k<n+m+o n+m≤k)) ⟩
          ⊎.rec (⊎.rec as bs ∘ finSplit n m) cs (inr (k ∸ (n + m) , assocr∘∸ k n m o k<n+m+o n+m≤k))
        ≡⟨ ⊎InrB (Fin (n + m)) ((⊎.rec as bs ∘ finSplit n m)) cs (k ∸ (n + m) , assocr∘∸ k n m o k<n+m+o n+m≤k) ⟩
          cs (k ∸ (n + m) , assocr∘∸ k n m o k<n+m+o n+m≤k)
        ≡⟨ congS cs (Σ≡Prop (λ _ -> isProp≤) (sym (∸+Assoc k n m))) ⟩
          cs (k ∸ n ∸ m , subst (_< o) (sym (∸+Assoc k n m)) (assocr∘∸ k n m o k<n+m+o n+m≤k))
        ≡⟨ ⊎InrB (Fin m) bs cs (k ∸ n ∸ m , subst (_< o) (sym (∸+Assoc k n m)) (assocr∘∸ k n m o k<n+m+o n+m≤k)) ⟩
          ⊎.rec bs cs (inr (k ∸ n ∸ m , subst (_< o) (sym (∸+Assoc k n m)) (assocr∘∸ k n m o k<n+m+o n+m≤k)))
        ...
        ≡⟨ ⊎InrB (Fin n) as (⊎.rec bs cs ∘ finSplit m o) (k ∸ n , ∸-<-lemma n (m + o) k k<n+m+o (m+n≤k→m≤k m n k n+m≤k)) ⟩
...
¬n<m<sucN : ∀ {n m} -> n < m -> m < suc n -> ⊥.⊥
¬n<m<sucN {n} {m} (x , p) (y , q) = znots lemma-β
  where
...
arrayA : sig M.MonSig (Array A) -> Array A
arrayA (M.`e , i) = e
arrayA (M.`⊕ , i) = i fzero ⊕ i fone
...
module Free {x y : Level} {A : Type x} {𝔜 : struct y M.MonSig} (isSet𝔜 : isSet (𝔜 .car)) (𝔜-monoid : 𝔜 ⊨ M.MonSEq) where
  ...
  module _ (f : A -> 𝔜 .car) where
    ...
    ♯Distr (suc n) xs m ys =
      ...
      ≡⟨ cong (f (xs fzero) 𝔜.⊕_) (♯Distr n _ m _) ⟩
  ...
  private
    ...
    arrayEquivLemmaβ : (g : structHom 𝔄 𝔜) -> g ≡ ♯IsMonHom (g .fst ∘ η)
    arrayEquivLemmaβ g = structHom≡ 𝔄 𝔜 g (♯IsMonHom (g .fst ∘ η)) isSet𝔜 (funExt λ (n , p) -> arrayEquivLemma g n p)

  arrayEquiv : structHom 𝔄 𝔜 ≃ (A -> 𝔜 .car)
  arrayEquiv =
    isoToEquiv (iso (λ g -> g .fst ∘ η) ♯IsMonHom (λ g -> funExt (𝔜.unitr ∘ g)) (sym ∘ arrayEquivLemmaβ))

module ArrayDef = F.Definition M.MonSig M.MonEqSig M.MonSEq

arrayStr : ∀ {n} (A : Type n) -> struct n M.MonSig
arrayStr A = < Array A , arrayA >
...
module _ {ℓ} {A : Type ℓ} where
  ...
  arrayIsoToListHom : structIsHom < Array A , arrayA > < List A , LM.listΑ > (arrayIsoToList .fun)
